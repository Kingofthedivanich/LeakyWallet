...
Этап 9 — аналитика и приватность: экран «Аналитика» (топ трат, разбивка по
категориям сервисов, тренд по месяцам, детектор «спящих» подписок — активна,
но давно нет чеков), кнопка «Как отменить» на карточке подписки (ссылка из
каталога), экспорт истории списаний в CSV и `/wipe` — необратимое удаление
всех данных пользователя (подписки, транзакции, напоминания, почтовый
аккаунт с токеном) с подтверждением, плюс экран «что мы храним» в настройках.
Категории сервисов — новое поле `services.category`, миграция
`4e6c9f00e7ca`. Стадии 5–6 (OAuth и скан почты) по-прежнему ждут вашей
проверки на реальном Google-аккаунте — см. ниже.
